...
function bumpCounterTable(data, workerId) {
  const c = parseInt(String(data.counter || "0"), 10) || 0;
  data.counter = String(c + 1);
  if (!Array.isArray(data.table_of_workers)) data.table_of_workers = [];
  data.table_of_workers.push(workerId);
}
...
function processMetricAttrs() {
  return {
    ...
    "demo.worker_id": envOr("DEMO_WORKER_ID", "js"),
  };
}
...
function pipelinePointAttrs(workerId) {
  return { ...processMetricAttrs(), worker_id: workerId };
}
...
async function startPyroscopeIfConfigured() {
  ...
  try {
    ...
    Pyroscope.init({
      serverAddress: srv,
      appName,
      tags: { demo_worker_id: envOr("DEMO_WORKER_ID", "js") },
    });
...
async function runPipelineOnce({
  ...
  const finishHop = () => {
    hopHist.record(Date.now() - t0, pipelinePointAttrs(workerId));
  };
  ...
  jsLine(`[${workerId}] received: ${JSON.stringify(msg)}`);
  ...
  await context.with(parentCtx, async () => {
    try {
      ...
      "pipeline.hop",
      { kind: SpanKind.SERVER, attributes: { "demo.worker_id": workerId } },
      parentCtx,
      async (hopSpan) => {
        ...
        try {
          ...
          if (String(seg.id || "").trim() !== workerId) {
            hopSpan.setStatus({ code: SpanStatusCode.ERROR });
            finishHop();
            res.writeHead(400);
            res.end(
              `first unvisited route segment id must match this node (expected ${JSON.stringify(workerId)}, got ${JSON.stringify(seg.id)})`
            );
            return;
          }
                ...
                if (st.kind === "cpu") {
                  ...
                  await tracer.startActiveSpan(
                    ...
                    async (stepSpan) => {
                      ...
                      jsLine(
                        `[${workerId}] nested_forward to ${nurl}: ${nestBody.toString()}`
                      );
          ...
          msg.visit_log.push(workerId);
          bumpCounterTable(msg, workerId);
          ...
          hopSpan.setAttribute(
            "demo.table_len",
            (msg.table_of_workers || []).length
          );
          hopSpan.setAttribute("demo.has_forward", nextIdx >= 0);

          if (nextIdx < 0) {
            jsLine(
              `[${workerId}] respond (terminal route): ${outBody.toString()}`
            );
            msgCounter.add(1, pipelinePointAttrs(workerId));
            finishHop();
          ...
          jsLine(
            `[${workerId}] forward to ${nextURL}: ${outBody.toString()}`
          );
...
async function main() {
  ...
  const workerId = envOr("DEMO_WORKER_ID", "js");
  ...
  const ts = new Date().toISOString().slice(11, 23);
  console.log(
    `${ts} pipeline: listen http://${addr}${httpPath} worker_id=${JSON.stringify(workerId)} peers=${JSON.stringify(Object.keys(peers))}`
  );
