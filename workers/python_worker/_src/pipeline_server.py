...
def _forward_to_next(url: str, body: bytes) -> tuple[int, bytes]:
    ...
    current_sc = current_span.get_span_context()
    if current_sc.is_valid:
        flags = "01" if current_sc.trace_flags.sampled else "00"
        traceparent = (
    ...
    if not carrier.get("traceparent") and os.environ.get(
        "DEMO_OTEL_FORWARD_DEBUG", ""
    ).strip().lower() in ("1", "true", "yes"):
        telemetry.log_pipeline_line(
            "OTel forward debug: brak traceparent po inject (downstream = nowy trace)"
        )
    elif os.environ.get("DEMO_OTEL_FORWARD_DEBUG", "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
        ...
        telemetry.log_pipeline_line(
            f"OTel forward debug: traceparent_len={len(tp)} prefix={tp[:55]!r}"
        )
    h = {
        **carrier,
        "Content-Type": "application/json",
        "User-Agent": "demo-pipeline/python",
    }
...
def run_server() -> None:
    ...
    if not telemetry.is_telemetry_ready():
        print(
            "[pipeline] FATAL: telemetry not ready after init_telemetry() "
            "(tracer/meter None — sprawdź OTEL_* i logi powyżej)",
            file=sys.stderr,
            flush=True,
        )
        raise SystemExit(1)
